...
void on_create(HWND hw)
{
	CreateWindow("BUTTON", "one", WS_CHILD | WS_VISIBLE,
		100, 50, 70, 30, hw, (HMENU)id_button1, GetModuleHandle(NULL), NULL);

	CreateWindow("BUTTON", "two", WS_CHILD | WS_VISIBLE,
		100, 100, 70, 30, hw, (HMENU)id_button2, GetModuleHandle(NULL), NULL);
}

void on_command(HWND hw, int id)
{
	if (id == id_button1) {
		MessageBox(hw, "1 pressed", "NWP", MB_OK | MB_ICONINFORMATION);
	}
	else if (id == id_button2) {
		MessageBox(hw, "2 pressed", "NWP", MB_OK | MB_ICONWARNING);
	}
}
